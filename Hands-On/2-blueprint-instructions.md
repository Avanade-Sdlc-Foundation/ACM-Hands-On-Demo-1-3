...
`/copilot-instructions-blueprint-generator` コマンドは、GitHub Copilotの高度な分析機能を活用します：

- **深いコード分析**: 実際のコード構造とパターンを詳細に解析
- **具体的な例**: 抽象的なルールではなく、プロジェクト固有のコード例を提供
- **実装レベルの詳細**: メソッドレベル、アノテーションレベルの仕様を含む
- **プロジェクト固有の検出**: 実際に使用されているライブラリやパターンを検出

前の演習のAgent機能との違いを実際に体験できます。
...
**Agent機能との違いに注目**:
- Agent機能: 一般的なSpringのルール
- ブループリント: プロジェクト固有の実装パターン
...
1. ファイルを保存します：
   - Copilotが自動でファイルを作成した場合は、`.github/prompt-copilot-instructions.md`に保存されていることを確認
   - 手動で保存する必要がある場合は、`.github`フォルダに`prompt-copilot-instructions.md`として保存
